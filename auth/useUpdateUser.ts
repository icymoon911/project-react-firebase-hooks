...
export const useUpdateEmail = (auth: Auth): UpdateEmailHook => {
  const [updateEmail, , loading, error] = useAction<
    [string],
    boolean,
    AuthError
  >(
  ...
  return [updateEmail as (email: string) => Promise<boolean>, loading, error];
};

export const useUpdatePassword = (auth: Auth): UpdatePasswordHook => {
  const [updatePassword, , loading, error] = useAction<
    [string],
    boolean,
    AuthError
  >(
  ...
  return [
    updatePassword as (password: string) => Promise<boolean>,
    loading,
    error,
  ];
};
...
export const useUpdateProfile = (auth: Auth): UpdateProfileHook => {
  const [updateProfile, , loading, error] = useAction<
    [Profile],
    boolean,
    AuthError
  >(
  ...
  return [
    updateProfile as (profile: Profile) => Promise<boolean>,
    loading,
    error,
  ];
};
...
export const useVerifyBeforeUpdateEmail = (
  ...
  const [verifyBeforeUpdateEmail, , loading, error] = useAction<
